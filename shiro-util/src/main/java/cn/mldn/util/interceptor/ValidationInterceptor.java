...
import cn.mldn.util.web.validation.ActionMIMEValidationUtil;
		...
		if (handler instanceof HandlerMethod) { // 判断对象是否为指定类的实例
			...
			if (validationRule != null) { // 程序里面需要进行验证处理
				...
				if (avu.getErrors().size() > 0) { // 此时出现了错误的信息，跳转到错误页
					...
					return false; // 进行拦截处理
				} else { // 基础验证已经通过了，应该进行上传文件的MIME验证
					if (request instanceof DefaultMultipartHttpServletRequest) { // 表单进行了封装
						String mimeRule = null; // 获取MIME验证规则
						try {
							mimeRule = this.messageSource.getMessage(validationKey + ".mime", null, null);
						} catch (Exception e) {
							mimeRule = this.messageSource.getMessage("mime.types", null, null);
						}
						ActionMIMEValidationUtil amvu = new ActionMIMEValidationUtil(request, this.messageSource,
								mimeRule);
